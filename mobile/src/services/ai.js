/**
 * src/services/ai.js
 * Dual-AI Smart Coach Chatbot & syllabus generator using Groq for React Native mobile.
 */

import AsyncStorage from '@react-native-async-storage/async-storage';
import { GROQ_API_KEY } from '../../js/config.js'; // Import shared API key from root
import { getProfile, getCurrentSubject, trackMistake } from './storage.js';

const GROQ_URL = 'https://api.groq.com/openai/v1/chat/completions';

async function getActiveGroqKey() {
  const saved = await AsyncStorage.getItem('vaaniai_settings');
  if (saved) {
    try {
...
async function callGroq(prompt) {
  const userKey = await getActiveGroqKey();
...
export async function sendChatMessage(currentUser, chatHistory, userMessage, imageData = null, coachPersonality = 'Friendly') {
  ...
  if (imageData) {
    ...
    try {
      ...
      result = {
        text: "I'm having trouble connecting to the AI right now. Please verify your free API keys in settings! 🔧",
        source: 'Error'
      };
    }
  }
